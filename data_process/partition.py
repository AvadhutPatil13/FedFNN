...
class FederatedPartition(object):
    ...
    def partition(self, gnd, is_shuffle=True, random_state=None):
        ...
        if self.partition_type == "homo":
            ...
            # Guarantee at least one test sample per client by redistributing if needed
            for j in range(self.n_client):
                np.random.shuffle(idx_batch[j])
                if len(idx_batch[j]) == 0:
                    # Find a donor client with >1 sample
                    donor = next((k for k in range(self.n_client) if len(idx_batch[k]) > 1), None)
                    if donor is not None:
                        idx_batch[j].append(idx_batch[donor].pop())
...
class FedKfoldPartition(object):
    ...
    def partition(self, gnd, is_shuffle=True, random_state=None):
        ...
        for client_i in range(self.args.n_client):
            ...
            n_test_loca = max(1, round(n_smpl_local / self.args.n_kfolds))  # Ensure at least 1 test sample
            local_train_indexes = []
            local_test_indexes = []
            for kfold_i in np.arange(self.args.n_kfolds):
                ...
                # Guarantee at least one test sample per client per fold
                if local_test_index.size == 0 and len(local_idx) > 0:
                    local_test_index = np.array([local_idx[0]])
    ...
    def record_local_data_stats(self, gnd):
        for cv_i in np.arange(self.args.n_kfolds):
            ...
            for test_local_i, test_local_dataidx_i in enumerate(self.local_test_indexes):
                idx = np.array(test_local_dataidx_i[cv_i], dtype=np.int64)
                unq, unq_cnt = np.unique(gnd[idx], return_counts=True)
        ...
        for cv_i in np.arange(self.args.n_kfolds):
            ...
            for train_local_i, train_local_dataidx_i in enumerate(self.local_train_indexes):
                idx = np.array(train_local_dataidx_i[cv_i], dtype=np.int64)
                unq, unq_cnt = np.unique(gnd[idx], return_counts=True)
                tmp = {unq[i]: unq_cnt[i] for i in range(unq.shape[0])}
...
# Test block to instantiate KFoldPartition with dummy data
if __name__ == "__main__":
    import numpy as np
    n_samples = 20
    n_folds = 4
    y = np.random.randint(0, 3, n_samples)
    kf = KFoldPartition(n_folds)
    kf.partition(y)
    train_idx, num_train = kf.get_train_indexes()
    test_idx, num_test = kf.get_test_indexes()
