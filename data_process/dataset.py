...
class FedDatasetCV(data.Dataset):
    ...
    def __build_truncated_dataset__(self, data_para, target_para):
        ...
        target_return = target_para
        if self.data_idxs is not None:
            idxs = np.array(self.data_idxs, dtype=np.int64)
            data_return = data_para[idxs]
            target_return = target_para[idxs].squeeze()
        return data_return, target_return
    ...
    def get_local_data(self, fold_idx=None):
        ...
        for test_local_i, test_local_dataidx_i in enumerate(self.fed_kfold_partition.local_test_indexes):
            ...
            print(f"[DEBUG] Client {test_local_i} fold {self.current_fold} test idx count: {len(local_test_idx)}")
            ...
            if local_test_idx.shape[0] > 0:
                ...
                print(f"[WARNING] Skipping DataLoader for client {test_local_i} (no test samples)")
        ...
        for train_local_i, train_local_dataidx_i in enumerate(self.fed_kfold_partition.local_train_indexes):
            ...
            print(f"[DEBUG] Client {train_local_i} fold {self.current_fold} train idx count: {len(local_train_idx)}")
            ...
            if local_train_idx.shape[0] > 0:
                ...
                print(f"[WARNING] Skipping DataLoader for client {train_local_i} (no train samples)")
...
def get_dataset_mat(dir_dataset, p_args):
    ...
    if p_args.nl > 0.0:
        ...
        print(f"[DEBUG] Added noise: nl={p_args.nl}, noise_num={noise_num}, shape={inputs.shape}")
