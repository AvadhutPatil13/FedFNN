...
def train_fm(comm_round=10, epochs=200, nl_levels=[0.0, 0.1, 0.2, 0.3]):
    results = []
    ...
    for nl in nl_levels:
        ...
        results.append(row)
        # Save per-level CSV
        pd.DataFrame([row]).to_csv(f'./results/fm/fm_result_{nl}.csv', index=False)
        print(f'Saved CSV: ./results/fm/fm_result_{nl}.csv')
        # Save interpretability values from .mat file to CSV as dataset_values_{nl}.csv
        import glob
        import scipy.io as sio
        mat_files = glob.glob(f'./results/interpretability/fm_fed_fpnn_csm_interpretability_r20c5p5_hetero1.0_nl{nl}*.mat')
        if mat_files:
                ...
                for k, v in mat.items():
                    if not k.startswith('__'):
                        if hasattr(v, 'flatten'):
                            ...
                            data[k] = str(v)
                pd.DataFrame([data]).to_csv(f'./results/fm/dataset_values_{nl}.csv', index=False)
                print(f'Saved values CSV: ./results/fm/dataset_values_{nl}.csv')
        ...
        import scipy.io as sio
        import matplotlib.pyplot as plt
        mat_summary_files = glob.glob(f'./results/fm/fm_fed_fpnn_csm_r20c5p5_hetero1.0_nl{int(nl*100)}_ce_lr0.0005_e10cr100.mat')
        if mat_summary_files:
            ...
            try:
                ...
                if acc is not None:
                    ...
                    plt.title(f'FM: Test Accuracy vs Communication Round (nl={nl})')
                    plt.legend()
                    plt.tight_layout()
                    plot_path = f'./results/fm/fm_acc_vs_comm_round_{nl}.pdf'
    ...
    plt.title('FM: Test Accuracy vs Uncertainty Level')
    plt.tight_layout()
    plot_path = './results/fm/fm_acc_vs_uncertainty.pdf'
    plt.savefig(plot_path)
    plt.close()
    print(f'Plotted and saved: {plot_path}')
