...
def train_wfrn(comm_round=10, epochs=200, nl_levels=[0.0, 0.1, 0.2, 0.3]):
    results = []
    python_exe = sys.executable
    os.makedirs('./results/wfrn', exist_ok=True)
    for nl in nl_levels:
        cmd = [
        ...
        results.append(row)
        # Save per-level CSV
        pd.DataFrame([row]).to_csv(f'./results/wfrn/wfrn_result_{nl}.csv', index=False)
        print(f'Saved CSV: ./results/wfrn/wfrn_result_{nl}.csv')
        # Save interpretability values from .mat file to CSV as dataset_values_{nl}.csv
        import glob
        import scipy.io as sio
        mat_files = glob.glob(f'./results/interpretability/wfrn_fed_fpnn_csm_interpretability_r20c5p5_hetero1.0_nl{nl}*.mat')
        if mat_files:
                ...
                for k, v in mat.items():
                    if not k.startswith('__'):
                        if hasattr(v, 'flatten'):
                            ...
                            data[k] = str(v)
                pd.DataFrame([data]).to_csv(f'./results/wfrn/dataset_values_{nl}.csv', index=False)
                print(f'Saved values CSV: ./results/wfrn/dataset_values_{nl}.csv')
        ...
        import matplotlib.pyplot as plt
        import glob
        mat_summary_files = glob.glob(f'./results/wfrn/wfrn_fed_fpnn_csm_r20c5p5_hetero1.0_nl{int(nl*100)}_ce_lr0.0005_e1cr3.mat')
        if mat_summary_files:
            ...
            try:
                ...
                if acc is not None:
                    ...
                    plt.title(f'WFRN: Test Accuracy vs Communication Round (nl={nl})')
                    plt.legend()
                    plt.tight_layout()
                    plot_path = f'./results/wfrn/wfrn_acc_vs_comm_round_{nl}.pdf'
        ...
        plt.title('WFRN: Test Accuracy vs Uncertainty Level')
        plt.tight_layout()
        plot_path = './results/wfrn/wfrn_acc_vs_uncertainty.pdf'
        plt.savefig(plot_path)
        plt.close()
        print(f'Plotted and saved: {plot_path}')
