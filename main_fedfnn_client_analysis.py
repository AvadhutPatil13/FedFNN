...
if __name__ == "__main__":
    ...
    print(f"[DEBUG] Running with nl={args.nl}")
    ...
    # --- Save summary .mat file for plotting script compatibility ---
    # Use actual training/test accuracy and loss tensors from FedAvgAPI
    # Debug: Print tensor shapes and sample values before saving
    def print_tensor_info(name, tensor):
        arr = tensor.cpu().numpy() if hasattr(tensor, 'cpu') else np.array(tensor)
        print(f"{name}: shape={arr.shape}, sample={arr.flatten()[:5]}")
        return arr

    save_dict_summary = {
        'global_test_acc_tsr': print_tensor_info('global_test_acc_tsr', fedavgAPI.global_test_acc_tsr) if hasattr(fedavgAPI, 'global_test_acc_tsr') else np.array([]),
        'global_test_loss_tsr': print_tensor_info('global_test_loss_tsr', fedavgAPI.global_test_loss_tsr) if hasattr(fedavgAPI, 'global_test_loss_tsr') else np.array([]),
        'global_train_loss_tsr': print_tensor_info('global_train_loss_tsr', fedavgAPI.global_train_loss_tsr) if hasattr(fedavgAPI, 'global_train_loss_tsr') else np.array([]),
        'global_train_acc_tsr': print_tensor_info('global_train_acc_tsr', fedavgAPI.global_train_acc_tsr) if hasattr(fedavgAPI, 'global_train_acc_tsr') else np.array([]),
    }
    results_dir = f'./results/{args.dataset}'
    if not os.path.exists(results_dir):
        os.makedirs(results_dir)
    # Save summary file with parameters matching plotting script and training command
    # Format nl as integer percentage (e.g., 0.0 -> 0, 0.1 -> 10, etc.)
    nl_int = int(round(args.nl * 100))
    summary_file = f'{results_dir}/{args.dataset}_fed_fpnn_csm_r{args.n_rule}c{args.n_client}p{args.n_client_per_round}_hetero{args.partition_alpha}_nl{nl_int}_{args.criterion}_lr{args.lr}_e{args.epochs}cr{args.comm_round}.mat'
    io.savemat(summary_file, save_dict_summary)
    print(f"Saved summary .mat for plotting: {summary_file}")
    ...
    for client_idx in range(args.n_client):
        if client_idx in fedavgAPI.train_data_local_dict:
            ...
            print(f"[WARNING] Skipping client {client_idx}: no train_data_local_dict entry.")
