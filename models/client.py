...
class FedFPNNClient:
    ...
    def _test_impl(self, test_data, device):
        ...
        # Debug: Check if test_data is empty or invalid
        try:
            first_batch = next(iter(test_data))
        except Exception as e:
            print(f"[WARNING] Skipping test for client due to empty or invalid test_data: {e}")
            metrics_normal['fs'] = fs
            return metrics_normal

        with torch.no_grad():
            for batch_idx, (x, target) in enumerate(test_data):
                if x is None or target is None or x.numel() == 0 or target.numel() == 0:
                    print(f"[WARNING] Skipping batch {batch_idx} due to empty x or target.")
                    continue
                x = x.to(device)
