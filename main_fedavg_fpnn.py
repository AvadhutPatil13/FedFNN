...
if __name__ == "__main__":
    ...
    for cv_idx in range(args.n_kfolds):
        ...
        # Get local data dicts for clients
        _, _, train_data_local_dict, test_data_local_dict = dataset.get_local_data()
        print("train_data_local_dict keys:", train_data_local_dict.keys())
        print("test_data_local_dict keys:", test_data_local_dict.keys())
        ...
        if not args.b_debug:
            wandb.init(
                ...
                name=str(args.model) + "-r" + str(args.n_rule) + "-c" + str(args.n_client) + "-p" + str(args.n_client_per_round) + '-' + str(args.partition_method) + str(args.partition_alpha) + "-nl" + str(args.nl) + "-" + args.criterion + "-lr" + str(args.lr) + "-cv" + str(cv_idx + 1),
                config=args
            )
